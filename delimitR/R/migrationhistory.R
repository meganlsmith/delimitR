...
generatemigratelist_old <- function(history, parsedtree,migrationmatrix){
  ...
  for(row in 1:nrow(migrationmatrix)){ # loop through rows
    for(col in 1:ncol(migrationmatrix)){ # and columns of the migration matrix
      if((row != col & !(paste(col,row,sep='_') %in% migchecked)) & (migrationmatrix[row,col] == TRUE)){ # check to see if the row matches the column (avoid migration between 1<->1, etc), and if col, row is already in the migchecked list.
        ...
        tstart <- NA # add NA for start time
        tstop <- NA # add NA for stop time
        migrationlistadd <- list(population1,population2,tstart,tstop) # create vector
        migrationlist[[i]] <- migrationlistadd # add vector to list
        i<- i+1 # increment counter
      }
    }
  }
  # next loop thorugh migration list and add the stop time (coalescent event for two lineages) where applicable
  for(migrationevent in 1:length(migrationlist)){
    FOUND <- FALSE # keep up with whether or not we added something
    pop1 <-migrationlist[[migrationevent]][1] # get pop 1 from migration event
    pop2 <-migrationlist[[migrationevent]][2] # get pop 2 from migration event
    for(item in 1:length(history)){ # loop through the history, and find a divergence time between pop 1 and pop 2 if it exists
      divevent <- strsplit(history[item],' ')[[1]][1] # get div event name
      divpop1 <- strsplit(history[item],' ')[[1]][2] # get pop 1 from the div event
      divpop2 <- strsplit(history[item],' ')[[1]][3] # get pop 2 from the div event
      if((divpop1 == pop1 & divpop2 == pop2) | (divpop1 == pop2 & divpop2 == pop1)){ # see if there are matches between the divergence pops and the migration pops
        migrationlist[[migrationevent]][4] <- divevent # if so, the div event is your stop time
        FOUND <- TRUE # and track that you found it by changing found to true
      }
    }
    if (FOUND == FALSE){ # if we didn't find one, then we were working with non-sister taxa. We can find the events that involve these taxa, and these are possible stop times
      possiblestops <- list() # create a list of possible stop times
      for(item in 1:length(history)){ # for each item in the history
        divevent <- strsplit(history[item],' ')[[1]][1] # get the div event name
        divpop1 <- strsplit(history[item],' ')[[1]][2] # get pop 1 from div event
        divpop2 <- strsplit(history[item],' ')[[1]][3] # get pop 2 from div event
        if(divpop1 == pop1 | divpop2 == pop2 | divpop1 == pop2 | divpop2 == pop1){ # see if there are any matches
          possiblestops <- c(possiblestops,divevent) # if so, add to the stop list
          if(divpop1 == pop1){
            pop1 <- divpop2
            for(item in 1:length(history)){ # for each item in the history
              divevent <- strsplit(history[item],' ')[[1]][1] # get the div event name
...
#          }
      ...
      newpossiblestops <- list()
      for(potentialstoptime in 1:length(possiblestops)){
        #print(possiblestops[[potentialstoptime]])
        if(!(possiblestops[[potentialstoptime]] %in% newpossiblestops)){
          newpossiblestops <- c(newpossiblestops,possiblestops[[potentialstoptime]])
        }
      }
      #print(newpossiblestops)
      possiblestops <- newpossiblestops
      migrationlist[[migrationevent]][4] <- list(possiblestops) # add all potential stop times to the migration list
    }
  }
  # Now, we can look for start times
  for(migrationevent in 1:length(migrationlist)){
    # get the info from the migration list
    pop1 <-migrationlist[[migrationevent]][1]
    pop2 <-migrationlist[[migrationevent]][2]
    tstop <- migrationlist[[migrationevent]][[4]]
    # create list to hold possible start times
    possiblestarts <- list()
    for(stoptime in 1:length(tstop)){ # look at each stop time
      currentstop = tstop[[stoptime]]
      stopnumber <- strsplit(currentstop,'v')[[1]][2]
      stopnumber <- gsub('$','',stopnumber,fixed=T) # get an integer value for the divergence time
      for(item in 1:length(history)){ # for each divergence event
        divevent <- strsplit(history[item],' ')[[1]][1] # get the div event name
        if(divevent == currentstop){ # if it is our current stop
          stopop1 <- strsplit(history[item],' ')[[1]][2] # then get stop pop 1
          stopop2 <- strsplit(history[item],' ')[[1]][3] # and get stop pop 2
        }
      }
      for(item in 1:length(history)){ # now we can loop through the history and find events between either of these populations that may occurr prior to our t stop
        divevent <- strsplit(history[item],' ')[[1]][1] # get div event name
        divpop1 <- strsplit(history[item],' ')[[1]][2] # get pop 1
        divpop2 <- strsplit(history[item],' ')[[1]][3] # get pop 2
        divnumber <- strsplit(divevent,'v')[[1]][2] # get div number
        divnumber <- gsub('$','',divnumber,fixed=T)
        if((divpop1 == stopop1 | divpop2 == stopop2 | divpop1 == stopop2 | divpop2 == stopop1) & divevent != currentstop & divnumber < stopnumber) { # if our pops match, and we aren't at our current stop divergence event, and divnumber is less than our stop number
          possiblestarts <- c(possiblestarts,divevent) # then this is a potential start time
        }
      }
    }
    migrationlist[[migrationevent]][3] <- list(possiblestarts) # add our start times to our migration list
  }
...
generatemighistory_old <- function(migrationlist, collapsedhistory,SC,DWG,nspec,theparsed, maxnmigrate){
    ...
    for(scdwg in range(1:2)){ # we need to consider secondary contact & divergence with gene flow
      ...
      if(scdwg == 1 & SC == TRUE){ # if the user wants secondary contact, and we're on those models
        ...
        for(potentialmigrationevent in 1:length(migrationlist)){
          ...
          migstop <- migration[[4]] # get stop times
      ...
      if(scdwg == 2 & DWG == TRUE){ # if the user wants dwg, and we're on those models
        ...
        for(potentialmigrationevent in 1:length(migrationlist)){
          ...
          #          cat('We are looking at migraiton between population ', migpop1, ' and population ', migpop2, '\n')
          for(divevent in 1:length(currenthistory)){
            divtime <- strsplit(currenthistory[[divevent]],' ')[[1]][1] # get div event name
            divpop1 <- strsplit(currenthistory[[divevent]],' ')[[1]][2] # get pop 1
            divpop2 <- strsplit(currenthistory[[divevent]],' ')[[1]][3] # get pop 2
            ...
            if(divtime!=0 & ((divpop1 == migpop1 & divpop2 == migpop2) | (divpop1 == migpop2 & divpop2 == migpop1)) & PRESENT == FALSE){ # see if the divtime is zero, and divpop 1 is either of our migration pops. Then, we should exclude the event
              currentstops <- migration[[4]]
              #cat('adding ', divtime, '\n')
              migration[[4]] <- c(currentstops,divtime)
          ...
          if(potentialcombonum < maxnmigrate+1){
            ...
            for(potentialcombo in 1:ncol(thiscombonum)){ # look at each combination in the column, which will correspond to a model
              ...
              if(length(currentmigrations) == 1){
                stoptime = currentmigrations[[1]][[1]][4][[1]]
                newstoptime = list()
                ...
                for(potentialdivtime in 1:length(currenthistory)){
                  #print(currenthistory[[potentialdivtime]])
                  divtime <- strsplit(currenthistory[[potentialdivtime]],' ')[[1]][1]
                  divpop1 <- strsplit(currenthistory[[potentialdivtime]],' ')[[1]][2]
                  divpop2 <- strsplit(currenthistory[[potentialdivtime]],' ')[[1]][3]
                  if(((divpop1 == migpop1 & divpop2 == migpop2) | (divpop2 == migpop1 & divpop1 == migpop2)) & divtime != 0){
                    stoptime = divtime
                  }
                }
                ...
                for(themigration in 1:length(currentmigrations)){
                  stoptime = currentmigrations[[themigration]][[1]][4]
                  newstoptime = list()
